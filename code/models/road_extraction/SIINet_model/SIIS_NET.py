...
class Resnet_SIIS(BasicModule):
    ...
    def __init__(self, num_classes=2,
        ...
        if resnet_arch == 'resnet50':
            encoder = resnet50(False, output_stride=self.output_stride)
...
class Deeplab_SIIS(BasicModule):
    ...
    def __init__(self, num_classes=2,
                 siis_size=[128, 128], width=1, kw=9, dim=128, arch=1,
                 resnet_arch='resnet50', output_stride=8, layer_num=2, pretrained_flag=True):
        ...
        if resnet_arch == 'resnet50':
            encoder = resnet50(pretrained_flag, output_stride=self.output_stride)
        elif resnet_arch == 'resnet101':
            encoder = resnet101(pretrained_flag, output_stride=self.output_stride)
        encoder = encoder._modules  # Covert class instance into orderdict
