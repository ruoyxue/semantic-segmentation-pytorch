...
class BuildPass_4(nn.Module):
    '''
    Sub_module of SIIS, complete the transfer of information on 4 paths.
    ...
    Args:
        ...
        kw - Conv kernel size
        Num - Actually the H or W of input feature map
    Notes：
    ...
    def forward(self, x):
        ...
        fm = torch.split(x, self.width, self.d+1)  # slice alonge H or W
        if len(fm) != self.num:
