...
from .resnet_d import resnet50, resnet101
...
class AlignNetResNetMaxAvgpool(nn.Module):
    def __init__(self, num_classes, trunk='resnet-101', variant='D', skip='m1', skip_num=48,
                 fpn_dsn=False, inplanes=128, reduce_dim=64, ignore_background=False,
                 max_pool_size=8, avgpool_size=8, edge_points=32, pretrained=True):
    ...
    def forward(self, x, gts=None):
        ...
        return main_out, edge_preds


def DeepR101_PF_maxavg_deeply(num_classes, reduce_dim=64, max_pool_size=8, avgpool_size=8, edge_points=32, pretrained=True):
    """
    ResNet-50 Based Network
    """
    return AlignNetResNetMaxAvgpool(num_classes, trunk='resnet-101-deep', variant='D', skip='m1',
                                    reduce_dim=reduce_dim, max_pool_size=max_pool_size, avgpool_size=avgpool_size,
                                    edge_points=edge_points, pretrained=pretrained)

def DeepR50_PF_maxavg_deeply(num_classes, reduce_dim=64, max_pool_size=8, avgpool_size=8, edge_points=32, pretrained=True):
    """
    ResNet-50 Based Network
    """
    return AlignNetResNetMaxAvgpool(num_classes, trunk='resnet-50-deep', variant='D', skip='m1',
                                    reduce_dim=reduce_dim, max_pool_size=max_pool_size, avgpool_size=avgpool_size,
                                    edge_points=edge_points, pretrained=pretrained)