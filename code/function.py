...
class FocalLoss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        loss_s = torch.where(loss > 0, loss, torch.zeros_like(loss).to(loss.device)).sum() + 1e-8
        loss_ori_s = torch.where(loss_ori > 0, loss_ori, torch.zeros_like(loss_ori).to(loss_ori.device)).sum() + 1e-8
